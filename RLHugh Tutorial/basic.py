...
if __name__ == "__main__":
    ...
    for i in range(episodes):
        ...
        while not game.is_episode_finished():
            ...
            #print(action_probs)
            # The largest logit is the aciton that the network is chosen

            # We should sample, but for now we just argmax
            _, action = action_logits.max(dim=-1) # Action is the index of the largest logit (probability)
            # What is dim=-1?
            action = action.item()
            print('Action: ', action)
        ...
        print("Episode ", (i + 1), " total reward =  ", game.get_total_reward())
